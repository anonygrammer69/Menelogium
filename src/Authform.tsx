...
const AuthForm: React.FC<{ onAuth: () => void }> = ({ onAuth }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-5 max-w-xs mx-auto text-black mt-10 p-10 bg-white rounded shadow-xl">
      ...
      <button
        type="button"
        className="bg-gray-300 text-black rounded p-2 hover:bg-gray-400 mt-2 login"
        onClick={handleGoogleSignIn}
      >
        <img src={googleLogo} alt="Google" className="h-6 w-6 justify-center google-logo" /> Sign in with Google
      </button>
    </form>
  );
};
