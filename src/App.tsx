...
function App() {
  ...
  return (
    <div className="w-full min-h-screen bg-transparent text-gray-900 dark:text-gray-100 transition-colors duration-300">
      ...
      >
        <span className="text-sm opacity-100">{theme === 'dark' ? 'Light ☀️' : 'Dark 🌙'}</span>
        <span
          className={`relative top-4 left-2 h-6 w-11 rounded-full transition-colors duration-300 ${theme === 'dark' ? 'bg-blue-500' : 'bg-gray-300'}`}
        >
          <span
